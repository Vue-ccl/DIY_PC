<template>
<div class="myforum">
    <van-nav-bar fixed placeholder title="我的发布" left-arrow @click-left="onClickLeft"></van-nav-bar>
   <div class="wrapper">
      <div class="scroll-wrapper">
          <publish-list v-for="(item,index) in myforum" :key="index" :index='index' :item='item' @pre-view='preView' @player-play='playerPlay' :isforum=2></publish-list>
      </div>
  </div>
  <!-- 显示图片预览栏 -->
...
import PublishList from '@/components/PublishList/PublishList'
    export default {
      ...
      watch:{
        myforum(){
          if(this.myforum.length<this.page*this.count){
          this.ismore=false
      ...
      methods: {
        ...
        reqpublishlist(){
          this.loading()
          this.reqMyForum({page:this.page,count:this.count,
            callback:(result)=>{
