...
export default {
    ...
    watch:{
        power(){
        this.goods= [].concat(...(Array.from(this.power.reduce((total, cur, index) => {
        total[index % 2].push(cur)
        return total
        }, { 0: [], 1: [], length: 2 }))))
        if(this.power.length<this.page*this.count){
        this.ismore=false
    ...
    reqhomehot(){
        this.loading()
        this.$store.dispatch('reqHomeHot',{page:this.page,count:this.count,type:'power',callback:()=>{this.$toast.clear()}})
    },
    pagecount(){
        this.page +=1
        this.reqhomehot()
